(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const t of document.querySelectorAll('link[rel="modulepreload"]'))s(t);new MutationObserver(t=>{for(const n of t)if(n.type==="childList")for(const i of n.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&s(i)}).observe(document,{childList:!0,subtree:!0});function o(t){const n={};return t.integrity&&(n.integrity=t.integrity),t.referrerPolicy&&(n.referrerPolicy=t.referrerPolicy),t.crossOrigin==="use-credentials"?n.credentials="include":t.crossOrigin==="anonymous"?n.credentials="omit":n.credentials="same-origin",n}function s(t){if(t.ep)return;t.ep=!0;const n=o(t);fetch(t.href,n)}})();class c{constructor(){this.apiKey="AIzaSyC6SsNshtIswEOiUVKTs1ZqRGdR9fOWx4s",this.apiUrl="https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent"}async generateTour(e,o){const s=this.createPrompt(e,o);try{if(this.shouldUseMockData())return{tourText:this.generateMockTour(e,o),prompt:s};const n={contents:[{role:"user",parts:[{text:s}]}]},i=await fetch(`${this.apiUrl}?key=${this.apiKey}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(n)});if(!i.ok){const l=await i.json();throw new Error(`API error ${i.status} - ${l.error?.message||i.statusText}`)}const a=await i.json();if(a.candidates?.[0]?.content?.parts?.[0]?.text)return{tourText:a.candidates[0].content.parts[0].text,prompt:s};throw new Error("Unexpected API response structure")}catch(t){throw console.error("Error generating tour:",t),t}}createPrompt(e,o){return`You are a local tour guide with loads of amazing reviews on your ability to create fun, immersive local tours.
...
**CONCLUSION:**
Our journey through ${e} reveals how a place becomes more than just buildings and streets – it becomes a living story written by everyone who calls it home. From the historic square where generations have gathered to the cozy café where new friendships bloom daily, we've seen how community spirit transforms ordinary spaces into extraordinary places. As you continue exploring ${e}, remember that every doorway, every corner, every friendly face has a story to tell. The real magic isn't in the grand monuments but in the daily rhythms of life that make this place uniquely special. Welcome to the neighborhood – you're now part of its ongoing story.`}}class h{parse(e){const o={title:"",duration:"",distance:"",startingPoint:"",notableStops:[],introduction:"",stops:[],journeys:[],conclusion:""};try{o.title=this.extractField(e,"TOUR TITLE")||"Hidden Gems Tour",o.duration=this.extractField(e,"DURATION")||"N/A",o.distance=this.extractField(e,"DISTANCE")||"N/A",o.startingPoint=this.extractField(e,"STARTING POINT")||"N/A";const s=this.extractField(e,"NOTABLE STOPS");s&&(o.notableStops=s.split(",").map(t=>t.trim())),o.introduction=this.extractSection(e,"INTRODUCTION","STOPS")||"Welcome to this hidden gems tour!",o.conclusion=this.extractSection(e,"CONCLUSION")||"Thank you for joining this hidden gems tour!",o.stops=this.parseStops(e),o.journeys=this.parseJourneys(e)}catch(s){throw console.error("Error parsing tour content:",s),new Error(`Failed to parse tour content: ${s.message}`)}return o}extractField(e,o){const s=new RegExp(`\\*\\*${o}:\\*\\*\\s*(.+)`,"i"),t=e.match(s);return t?t[1].trim():null}extractSection(e,o,s=null){let t;s?t=new RegExp(`\\*\\*${o}:\\*\\*\\s*([\\s\\S]+?)(?=\\*\\*${s}:\\*\\*)`,"i"):t=new RegExp(`\\*\\*${o}:\\*\\*\\s*([\\s\\S]+?)$`,"i");const n=e.match(t);return n?n[1].trim():null}parseStops(e){const o=[],s=/\*\*Stop (\d+): ([^*]+)\*\*\s*([\s\S]*?)(?=\*\*(?:Stop \d+:|Journey to Stop \d+:|CONCLUSION:)|$)/gi;let t;for(;(t=s.exec(e))!==null;){const n={number:parseInt(t[1]),name:t[2].trim(),description:"",coordinates:null,pexelsPhotoId:null,hiddenHistory:"",localTip:""},i=t[3].trim();n.description=this.extractStopDetail(i,"Description"),n.pexelsPhotoId=this.extractStopDetail(i,"Pexels Photo ID"),n.youAreHere=this.extractStopDetail(i,"You Are Here"),n.theHook=this.extractStopDetail(i,"The Hook"),n.fascinatingFacts=this.extractStopDetail(i,"Fascinating Facts"),n.storiesVoices=this.extractStopDetail(i,"Stories & Voices"),n.interactiveMoment=this.extractStopDetail(i,"Interactive Moment"),n.photoOp=this.extractStopDetail(i,"Photo Op"),n.hiddenHistory=this.extractStopDetail(i,"Hidden History"),n.localTip=this.extractStopDetail(i,"Local Tip"),n.coordinates=this.extractCoordinates(i),o.push(n)}return o}parseJourneys(e){const o=[],s=/\*\*Journey to Stop (\d+):\*\*\s*([\s\S]*?)(?=\*\*Stop \d+:|$)/gi;let t;for(;(t=s.exec(e))!==null;)o.push({toStop:parseInt(t[1]),description:t[2].trim()});return o}extractStopDetail(e,o){const s=new RegExp(`- \\*\\*${o}:\\*\\*\\s*([^-]+)`,"i"),t=e.match(s);return t?t[1].trim():""}extractCoordinates(e){const o=e.match(/- \*\*Coordinates:\*\*\s*\[([^,]+),\s*([^\]]+)\]/i);return o?{latitude:parseFloat(o[1].trim()),longitude:parseFloat(o[2].trim())}:null}}class d{constructor(){this.bindElements()}bindElements(){this.elements={tourTitle:document.getElementById("tourTitle"),summaryDuration:document.getElementById("summaryDuration"),summaryDistance:document.getElementById("summaryDistance"),summaryStartingPoint:document.getElementById("summaryStartingPoint"),summaryNotableStops:document.getElementById("summaryNotableStops"),tourIntro:document.getElementById("tourIntro"),tourStopsContainer:document.getElementById("tourStopsContainer"),tourConclusion:document.getElementById("tourConclusion")}}render(e){this.renderHeader(e),this.renderSummary(e),this.renderIntroduction(e),this.renderStops(e),this.renderConclusion(e)}renderHeader(e){this.elements.tourTitle.textContent=e.title}renderSummary(e){this.elements.summaryDuration.textContent=e.duration,this.elements.summaryDistance.textContent=e.distance,this.elements.summaryStartingPoint.textContent=e.startingPoint,this.elements.summaryNotableStops.innerHTML="",e.notableStops.forEach(o=>{const s=document.createElement("li");s.textContent=o,this.elements.summaryNotableStops.appendChild(s)})}renderIntroduction(e){this.elements.tourIntro.textContent=e.introduction}renderStops(e){this.elements.tourStopsContainer.innerHTML="",e.stops.forEach((o,s)=>{this.renderTourStop(o);const t=e.journeys.find(n=>n.toStop===o.number+1);t&&this.renderJourneyBetweenStops(t.description)})}renderTourStop(e){const o=document.createElement("div");o.className="tour-stop";const s=e.pexelsPhotoId||"2387873",t=`https://images.pexels.com/photos/${s}/pexels-photo-${s}.jpeg?auto=compress&cs=tinysrgb&w=600&h=400&fit=crop`,n="https://images.pexels.com/photos/2387873/pexels-photo-2387873.jpeg?auto=compress&cs=tinysrgb&w=600&h=400&fit=crop";o.innerHTML=`
    ...
    `,this.elements.tourStopsContainer.appendChild(o)}renderStorySection(e){const o=e.theHook&&e.theHook.trim(),s=e.storiesVoices&&e.storiesVoices.trim();if(!o&&!s)return"";let t="";return o&&(t+=`<p class="hook-content">${e.theHook}</p>`),s&&(t+=`<blockquote class="stories-content">${e.storiesVoices}</blockquote>`),`<div class="story-section"><div class="section-icon">📖</div><div class="section-title">The Story</div>${t}</div>`}renderCaptureTheMomentSection(e){const o=e.interactiveMoment&&e.interactiveMoment.trim(),s=e.photoOp&&e.photoOp.trim();if(!o&&!s)return"";let t="";return o&&(t+=`<div class="interactive-content"><strong>Experience:</strong> ${e.interactiveMoment}</div>`),s&&(t+=`<div class="photo-content"><strong>Photo Opportunity:</strong> ${e.photoOp}</div>`),`<div class="capture-moment-section"><div class="section-icon">📸</div><div class="section-title">Capture the Moment</div>${t}</div>`}formatFacts(e){let o=[];return e.includes("•")?o=e.split("•").filter(s=>s.trim()).map(s=>s.trim()):e.includes("- ")?o=e.split(/- /).filter(s=>s.trim()).map(s=>s.trim()):e.includes(`
`)?o=e.split(`
`).filter(s=>s.trim()).map(s=>s.trim().replace(/^[-•]\s*/,"")):o=e.split(/\.(?=\s[A-Z])/).filter(s=>s.trim()).map(s=>{const t=s.trim();return t.endsWith(".")?t:t+"."}),o=o.filter(s=>s.length>3),`<ul class="facts-list">${o.map(s=>`<li>${s}</li>`).join("")}</ul>`}renderMapLink(e){return`
    ...
    `}renderJourneyBetweenStops(e){const o=document.createElement("div");o.className="tour-journey-between-stops",o.innerHTML=`<strong>Along the Way:</strong> ${e}`,this.elements.tourStopsContainer.appendChild(o)}renderConclusion(e){this.elements.tourConclusion.textContent=e.conclusion}}class u{constructor(){this.map=null,this.infoWindow=null,this.markers=[]}initMap(e,o){if(console.log("MapController: initMap called with stops:",o),!window.google||!window.google.maps){console.error("Google Maps API not loaded");return}try{const s={lat:37.7749,lng:-122.4194};this.map=new google.maps.Map(document.getElementById(e),{zoom:10,center:s,mapTypeId:"roadmap"}),this.infoWindow=new google.maps.InfoWindow,console.log("Map initialized successfully"),o&&o.length>0?this.addMarkers(o):console.warn("No stops provided to display on map")}catch(s){console.error("Error initializing map:",s)}}addMarkers(e){console.log("Adding markers for stops:",e),this.clearMarkers();const o=new google.maps.LatLngBounds;let s=0;e.forEach((t,n)=>{if(t.coordinates&&t.coordinates.latitude&&t.coordinates.longitude){const i={lat:t.coordinates.latitude,lng:t.coordinates.longitude},a=new google.maps.Marker({position:i,map:this.map,title:t.name||`Stop ${t.number}`,animation:google.maps.Animation.DROP,label:{text:t.number?t.number.toString():(n+1).toString(),color:"white",fontWeight:"bold"}});a.addListener("click",()=>{const l=`
